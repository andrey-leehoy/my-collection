...
function App() {
        ...
        {images.map((src, index) => {
          ...
          return (
            <div key={index} onMouseEnter={() => setHoveredIndex(index)} onMouseLeave={() => setHoveredIndex(null)} onClick={() => { setCurrentIndex(index); setViewMode("album"); }} style={{ position: "relative", width: "100%", maxWidth: "var(--funko-item-max-width)", height: "var(--funko-item-height)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-end", cursor: "pointer" }}>
              ...
              <img src={encodeURI(src)} alt={title} className="funko-shelf-img" style={{ width: "var(--funko-img-width)", height: "auto", maxHeight: "var(--funko-img-max-height)", objectFit: "contain", zIndex: 2, transition: "all 0.4s ease", transform: isHovered ? "scale(1.08) translateY(-15px)" : "scale(1)", filter: isHovered ? "drop-shadow(0 0 25px rgba(255,255,255,0.5)) brightness(1.1)" : "var(--funko-img-filter)" }} />
  ...
  const WishlistPinterest = () => {
    return (
      <div className="wishlist-grid" style={{ display: "grid", gridTemplateColumns: "repeat(var(--wishlist-cols), 1fr)", gap: "var(--wishlist-gap)", width: "100%", maxWidth: "1200px", margin: "0 auto" }}>
        {data.wishlist.map((item, index) => {
          const isReserved = reservedWishes.includes(item.id);
          return (
            <div key={item.id} className="wishlist-card" onClick={() => setSelectedWish(item)} onMouseEnter={() => setHoveredIndex(index)} onMouseLeave={() => setHoveredIndex(null)} style={{ borderRadius: "var(--card-radius)", overflow: "hidden", cursor: "pointer", position: "relative", transition: "all 0.3s ease", transform: hoveredIndex === index ? "scale(1.02)" : "scale(1)", boxShadow: "0 10px 30px rgba(0,0,0,0.3)", background: "#1a1a20", opacity: isReserved ? 0.4 : 1 }}>
              <img src={encodeURI(item.image)} alt={item.title} className="wishlist-card-img" style={{ width: "100%", display: "block", aspectRatio: "1", objectFit: "cover", objectPosition: "center" }} />
              {isReserved && (
    ...
    <div className="app-responsive" style={{ background: "#0f0f14", minHeight: "100vh", width: "100%", color: "white", fontFamily: "system-ui, sans-serif" }}>
                      ...
                      {images.map((src, i) => {
                        ...
                        return (
                          <div key={i} onClick={() => { setCurrentIndex(i); setViewMode("album"); }} style={{ width: "var(--funko-carousel-item-width)", display: "flex", flexDirection: "column", alignItems: "center", cursor: "pointer", flexShrink: 0, position: "relative", overflow: "visible" }}>
                            ...
                            <img src={encodeURI(src)} alt={title} style={{ width: "var(--funko-carousel-img-width)", height: "var(--funko-carousel-img-height)", objectFit: "contain", zIndex: 2, transition: "all 0.6s ease", filter: isSelected ? "drop-shadow(0 0 40px rgba(255,255,255,0.4)) brightness(1.1)" : "var(--funko-carousel-unselected-filter)", transform: isSelected ? "scale(1.3) translateY(-30px)" : "scale(0.85)" }} />
                                ...
                                {activeTab === "vinyl" && absOffset === 0 && (
                                  <div style={{ marginTop: 15, borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 15, opacity: (isHoveringAlbum || "ontouchstart" in window) ? 1 : 0, transform: (isHoveringAlbum || "ontouchstart" in window) ? "translateY(0)" : "translateY(20px)", transition: "all 0.4s ease", maxHeight: (isHoveringAlbum || "ontouchstart" in window) ? "300px" : "0", overflow: "hidden" }}>
                                    <div style={{ display: "flex", alignItems: "center", gap: 15, marginBottom: 15 }}>
                                      <button onClick={(e) => { e.stopPropagation(); setIsPlaying(!isPlaying); }} className="vinyl-play-btn" style={{ background: "#fff", border: "none", borderRadius: "50%", width: 44, height: 44, minWidth: 44, minHeight: 44, aspectRatio: "1", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", outline: "none", WebkitTapHighlightColor: "transparent" }}>
      ...
      {selectedWish && (
        <div
          ...
          style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", backdropFilter: "blur(20px) saturate(180%)", zIndex: 2000, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", overflowY: "auto", overflowX: "hidden", touchAction: "pan-y" }}
        >
          <div onClick={(e) => e.stopPropagation()} className="app-modal-inner" style={{ background: "rgba(30, 30, 35, 0.7)", backdropFilter: "blur(40px) saturate(200%) brightness(1.2)", WebkitBackdropFilter: "blur(40px) saturate(200%) brightness(1.2)", borderRadius: "40px", width: "100%", maxWidth: "1000px", maxHeight: "95vh", display: "flex", overflowY: "auto", overflowX: "hidden", boxShadow: "0 40px 100px rgba(0,0,0,0.6), inset 0 0 0 1px rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.1)", position: "relative", touchAction: "pan-y" }}>
            <button onClick={() => setSelectedWish(null)} aria-label="Закрыть" className="modal-close-btn" style={{ position: "absolute", top: 16, right: 16, width: 44, height: 44, minWidth: 44, minHeight: 44, borderRadius: "50%", border: "none", background: "rgba(255,255,255,0.15)", color: "#fff", fontSize: 24, lineHeight: 1, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 10, outline: "none", WebkitTapHighlightColor: "transparent" }}>×</button>
            <div className="modal-img-container" style={{ width: "var(--modal-img-width)", minWidth: 0, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", padding: "var(--modal-img-padding)", position: "relative", minHeight: "200px", overflow: "hidden" }}>
              <img src={encodeURI(selectedWish.image)} alt={selectedWish.title} className="modal-wish-img" style={{ maxWidth: "100%", maxHeight: "70vh", width: "auto", height: "auto", objectFit: "contain", objectPosition: "center", borderRadius: "24px", filter: "drop-shadow(0 30px 60px rgba(0,0,0,0.5))", zIndex: 2, display: "block" }} />
            </div>
